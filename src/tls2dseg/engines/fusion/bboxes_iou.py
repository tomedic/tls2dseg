"""AABB and OBB IoU helpers — moved verbatim from tls2dseg.bboxes_iou (D-D-03).

Phase 4 plan 03 Task 1 (ENG-07). This is the canonical location for these
functions per D-D-03 (verbatim move; current impl stays canonical).

* ``compute_aabb_iou_vectorized`` — pure numpy; tier_a safe (no trimesh/scipy).
* ``compute_obb_iou_naive`` — OBB IoU via trimesh boolean; tier_b_light.
* ``compute_obb_iou_parallel`` — parallel OBB IoU via ProcessPoolExecutor; tier_b_light.

Import-cycle rule (RESEARCH Pitfall 2): this module imports ONLY
numpy, scipy.spatial.transform, trimesh, and stdlib. It MUST NOT import
...
logger = logging.getLogger("tls2dseg.engines.fusion.bboxes_iou")
