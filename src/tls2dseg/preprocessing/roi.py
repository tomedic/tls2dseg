"""ROI masking helpers — pure numpy, tier_a testable.

Phase 4 plan 05 Task 1 (ENG-07, D-D-01). Verbatim move from
``tls2dseg.roi_filter`` (roi_mask_xy, roi_mask_xy_rectaware + three private
helpers).  Only the module docstring and logger are new; all function bodies
are unchanged (PATTERNS.md: verbatim move).

Imports: numpy only — this module is tier_a importable without pchandler/pc2img.

Closest analog: ``tls2dseg.roi_filter`` (exact source).
"""
...
logger = logging.getLogger("tls2dseg.preprocessing.roi")
...
def roi_mask_xy_rectaware(
    ...
    - Else: falls back to ``roi_mask_xy``.
    """
    ...
    # Fallback to general polygon routine (ray crossing)
    else:
