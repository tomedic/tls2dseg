...
def pc2img_run(pcd:PointCloudData, pcd_path: Path, rotate_pcd: bool = False, theta: float|int = 0,
               image_height: int = 1000, image_width: int = 1000, rasterization_method: str = 'nanconv',
    ...
    # Saving results in a list tuples with: feature_name (str), image (NDArray[np.uint8]), path_to_saved_image (Path)
    images_i = []
    for f in features:
        ...
        save_image_path = Path(f"./results/{pcd_path.stem}_Spherical_{f}.png")
        iio.imwrite(save_image_path, spherical_image)
        images_i.append((f, spherical_image, save_image_path))
    return images_i
