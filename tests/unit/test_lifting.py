...
Phase 4 plan 04-04 Task 3 (ENG-07).  After 04-04 Task 3,
``img_1to3_channels_encoding`` lives in ``tls2dseg.engines.inference.shared``
(pure numpy, no heavy deps) and all four encoding tests are ``tier_a``.

Locks the tls2dseg-side rasterization helper
...
from tls2dseg.engines.inference.shared import img_1to3_channels_encoding
...
#
# Upgraded to tier_a after 04-04 Task 3 moved the function to
# tls2dseg.engines.inference.shared (pure numpy, no heavy deps).
# ------------------------------------------------------------------
